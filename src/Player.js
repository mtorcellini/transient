...
export default class Player {
	constructor(){
		this.play = {
			...
			"kick": () => {
				this.metronome.play();
				setTimeout(()=>{this.metronome.stop()}, 250);
			},
			"clap": () => {
				this.clap.play();
		...
		this.tones = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
		this.oct = [2, 3, 4, 5, 6];
		this.metronome = instrument('sine', 130.8, 0.1, 0.6, 0);
		this.clap = instrument('sawtooth', 261.6, 0.03, 1.5, 0);
		this.voices = [{
			voiceName: "soprano",
		...
		/* Additional setup */
		this.metronome.addEffect(this.dubDelay);
	...
	randNoteForVoice(voice, range, baseOctave){
		let note = {
			...
			root: this.tones[Math.floor(Math.random() * this.tones.length)].substring(0, 1),
	...
	rand_oct() {
            ...
            var v = 1 - Math.random();
            return Math.abs(Math.floor(Math.sqrt(-2.0 * Math.log(u)) * Math.cos(2.0 * Math.PI * v) * 2)) + 1;
        }
        noteToFreq(numSteps) {
